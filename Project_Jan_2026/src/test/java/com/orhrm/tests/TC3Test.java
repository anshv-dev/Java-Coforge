...
public class TC3Test {
  ...
  public void tC3() throws Exception {
    ...
    // Scroll a bit to ensure product cards are in view
    js.executeScript("window.scrollTo(0, 600)");
    test.addScreenCaptureFromPath(takeScreenshot("scrolled"));

    // Click "Add to Wish List" on a known product (e.g., iPhone) using a stable selector
    // Product card: div.product-thumb contains h4/a with product name; its wish button has data-original-title
    ...
    // Wait for the success alert (or the login-required alert) and capture it
    ...
    // Go to Wishlist from the header link
    ...
    // On wishlist page, if user isn't logged in, site may redirect to login or show empty list.
    // Try to find the table rows. If none, log and exit gracefully.
    ...
    // If table exists, check if there is at least one row with remove button
